...
import {
    ...
    timeToTwoDigits,
    getTotalTimeWithOtherHalfInMinutes,
    ...
    const onMouseMove = (
        ...
        if (mouseAction === MOUSE_ACTION.SELECT) {
            ...
            if (eventCopy.spiltedReferenceId) {
                const otherHalf: Event = { ...getEventById(eventCopy.spiltedReferenceId)! }
                totalMin = getTotalTimeWithOtherHalfInMinutes(eventCopy, otherHalf)
            }
            if (totalMin <= 1240) {
            ...
            if (!eventCopy.endDate && bucketIndex !== undefined && !eventCopy.spiltedReferenceId) {
